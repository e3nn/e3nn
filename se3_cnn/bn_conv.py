...
class SE3BNConvolution(torch.nn.Module):
    ...
    def __init__(self, size, n_radial, Rs_in, Rs_out, upsampling=15, central_base=True, verbose=True, eps=1e-5, momentum=0.1, mode='normal', **kwargs):
        super().__init__()
        self.bn = SE3BatchNorm([(m, SO3.dim(R)) for m, R in Rs_in], eps=eps, momentum=momentum, mode=mode)
        self.conv = SE3Convolution(size, n_radial, Rs_in=Rs_in, Rs_out=Rs_out, upsampling=upsampling, central_base=central_base, verbose=verbose, **kwargs)
