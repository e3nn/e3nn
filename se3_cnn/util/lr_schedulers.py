def lr_scheduler_exponential(optimizer, epoch, init_lr, epoch_start, base_factor, verbose=False):
	"""
...
def lr_scheduler_step(optimizer, epoch, init_lr, decay_epochs, decay_factors, verbose=False):
	"""
	...
	for e,f in zip(decay_epochs, decay_factors):
		if epoch>=e:
			...
			break
	if verbose: print('learning rate = {:6f}'.format(lr))
	for param_group in optimizer.param_groups:
		param_group['lr'] = lr
	return optimizer, lr