...
def test_empty():
    x = o3.from_chunks("", [], (2, 2), torch.float32)
    assert x.irreps == o3.Irreps([])
    assert x.shape == (2, 2, 0)


def test_convert():
    id = o3.from_chunks("10x0e + 10x0e", [None, torch.ones((1, 10, 1))], (1,))
    assert tree_map(
        _shape, id.rechunk("0x0e + 20x0e + 0x0e").chunks
    ) == [None, (1, 20, 1), None]
    assert tree_map(
         _shape, id.rechunk("7x0e + 4x0e + 9x0e").chunks
    ) == [None, (1, 4, 1), (1, 9, 1)]

    id = o3.from_chunks("10x0e + 10x1e", [None, torch.ones((1, 10, 3))], (1,))
    assert tree_map(
        _shape, id.rechunk("5x0e + 5x0e + 5x1e + 5x1e").chunks
    ) == [
        None,
        None,
        (1, 5, 3),
    ...
    b = o3.from_chunks(b.irreps, b.chunks, b.shape[:-1])

    torch.testing.assert_allclose(a.array, b.array)


def test_indexing():
    x = o3.IrrepsArray("2x0e + 1x0e", torch.Tensor([[1.0, 2, 3], [4.0, 5, 6]]))
    assert x.shape == (2, 3)
    ...
    x = o3.IrrepsArray("2x0e + 1x2e", torch.arange(3 * 4 * 7).reshape((3, 4, 7)))
...
def test_indexing2():
    x = o3.IrrepsArray("2x0e + 1x0e", torch.ones((2, 2, 3)))
    y = x[0]
    assert y.shape == (2, 3)
...
def test_reductions():
    x = o3.IrrepsArray(
        "2x0e + 1x1e", torch.Tensor([[1.0, 2, 3, 4, 5], [4.0, 5, 6, 6, 6]])
    )
    assert o3.sum(x).irreps == o3.Irreps("0e + 1e")
    np.testing.assert_allclose(o3.sum(x).array, torch.Tensor([12.0, 9, 10, 11]))
    np.testing.assert_allclose(
        o3.sum(x, axis=0).array, torch.Tensor([5.0, 7, 9, 10, 11])
    )
    np.testing.assert_allclose(
        o3.sum(x, axis=1).array, torch.Tensor([[3.0, 3, 4, 5], [9.0, 6, 6, 6]])
    )

    np.testing.assert_allclose(
        o3.mean(x, axis=1).array, torch.Tensor([[1.5, 3, 4, 5], [4.5, 6, 6, 6]])
    )
...
def test_operators():
    ...
    y = o3.IrrepsArray(
        "2x0e + 1x1o", torch.Tensor([[1.0, 2, 3, 4, 5], [4.0, 5, 6, 6, 6]])
    )
    ...
    np.testing.assert_allclose(o3.norm(x / o3.norm(x)).array, 1)
