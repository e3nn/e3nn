...
def run_benchmark(irreps_type):
    ...
    with torch.no_grad():
        for lmax in range(1, LMAX + 1):
            ...
            irreps_out = (irreps).regroup()
            print(f"{irreps_type.upper()}: {irreps_in} -> {irreps_out}")

            linear = module.Linear(irreps_in, irreps_out)

            linear_compile = torch.compile(linear, mode=compile_mode).to(device=device)
            print(
            ...
            linear_experimental = experimental.Linear(irreps_in, irreps_out)

            linear_experimental_compile = torch.compile(linear_experimental, mode=compile_mode, fullgraph=True).to(
                device=device
            )
            print(
                f"{irreps_type.upper()} Linear Experimental lmax {lmax} channel {CHANNEL} batch {BATCH}: {print_performance(lambda: linear_experimental_compile(x), times=100, repeat=10)*1000:.3f}ms"
            )
