...
def test_sparsefulltp(irreps_in1, irreps_in2):
    x = o3.Irreps(irreps_in1).randn(2, -1)
    y = o3.Irreps(irreps_in2).randn(2, -1)

    tp_pt2 = torch.compile(
        o3.experimental.FullTensorProductSparse(irreps_in1, irreps_in2, leading_shape=(2,)), fullgraph=True, disable=True
    )
